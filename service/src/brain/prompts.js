...
Grammar: every card is a title line (max 40 chars; 2 to 5 words except the social captions described below, which may be a short sentence) plus at most one subtitle line (under 9 words, max 70 chars). No exclamation marks. No emoji. Write normal case, never ALL CAPS: Title Case for the status and the address, sentence case for captions; the renderer applies small caps where a layout wants them.
- hook_card: by title_card_system "${format.title_card_system}". price_stack: price, then address line, then beds | baths | area with basis label. status_card: title is the status in Title Case and nothing else ("Just Listed", "Coming Soon", "Open House", "Price Improved"), subtitle is the street address, city and region ("16 Foxwood Blvd, Mechanicsburg, PA"); the beds, baths and area line under it is added by the code from the form, so leave it out. number_first: "What {price} gets you in {locality}". search_intent: "Homes for sale in {locality}". address_only: address only.
- proof_card: one special from specials with its source, written in the same social-caption voice as fact_captions.
- fact_captions: one per shot whose caption_slot is "fact", each carrying that shot_id. The title is a short social caption in the voice of someone scrolling who just saw that room, built on ONE real feature of it: a question plus a reaction, or a single line ("Indoor salt water pool? Say less.", "Twelve-seat media room. Movie night.", "Two kitchens. No fights over the stove."). 40 characters maximum for the title; put the rest in the subtitle if it needs one. It may speak as "I" or to "you" about the HOUSE; it never describes who should live there. The feature must come from provable_numbers or observed items, traced in claims_trace like any other claim. A caption about a room goes ONLY on a shot of that room_class: a kitchen fact on a kitchen shot, a facade or porch fact on an exterior shot. If no listed shot is that room, leave the fact out. Whole-property numbers (price, beds, baths, area, year) do not go on a single room.
